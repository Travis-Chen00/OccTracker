# Author:   Boyu Chen
# This file serves as the logger for information
import json
import os
import yaml
import argparse
...
class ProgressLogger:
    ...
    def update(self, step_len: int, **kwargs):
        if (self.only_main and is_main_process()) or (self.only_main is False):
...
class Logger:
    ...
    def __init__(self, logdir: str, only_main: bool = True):
        ...
        if (self.only_main and is_main_process()) or (self.only_main is False):
            ...
            self.logdir = None
            self.tb_iters_logger = None
            self.tb_epochs_logger = None
        return

    def show(self, head, log):
        if (self.only_main and is_main_process()) or (self.only_main is False):
            print(f"{head} {log}")
        else:
            pass
    ...
    def write(self, head, log, filename, mode):
        """
    ...
    def tb_add_metric_log(self, log, steps, mode):
        if (self.only_main and is_main_process()) or (self.only_main is False):
