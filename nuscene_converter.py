...
NameMapping = {
    ...
    'object.trafficcone': 'traffic_cone',
    'movable_object.trafficcone': 'traffic_cone',
}

def points_cam2img(points_3d, cam_intrinsic, with_depth=False):
    assert points_3d.shape[-1] == 3
    pts = points_3d @ cam_intrinsic.T
    u = pts[..., 0] / pts[..., 2]
    v = pts[..., 1] / pts[..., 2]
    if with_depth:
        return np.stack([u, v, pts[..., 2]], axis=-1)
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in tqdm(nusc.sample, desc="Processing samples"):
        ...
        if not os.path.exists(lidar_path):
            raise FileNotFoundError(f"{lidar_path} not found.")
...
def export_2d_annotation(root_path, info_path, version, mono3d=True):
    ...
    for info in tqdm(nusc_infos):
        for cam in camera_types:
            ...
            coco_infos = get_2d_boxes(
                ...
                mono3d=mono3d)
            img = cv2.imread(cam_info['data_path'])  # BGR
    ...
    # with open(f'{json_prefix}.coco.json', 'wb') as f:
    #     pickle.dump(coco_2d_dict, f)
